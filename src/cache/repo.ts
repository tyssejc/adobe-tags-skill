...
export function upsertResource(db: Database, r: ResourceRow): void {
  db.query(`INSERT INTO resources (id, type, name, enabled, deleted, dirty, delegate_descriptor_id, head_revision_number, head_settings_json, updated_at, search_text)
    VALUES ($id, $type, $name, $enabled, $deleted, $dirty, $ddi, $rev, $settings, $updated, $search)
    ON CONFLICT(id) DO UPDATE SET name=$name, enabled=$enabled, deleted=$deleted, dirty=$dirty, delegate_descriptor_id=$ddi,
...
export interface UnpublishedRow { id: string; name: string; type: string; updated_at: string | null; }

export function unpublishedResources(db: Database): UnpublishedRow[] {
  return db.query(`SELECT id, name, type, updated_at
                   FROM resources
                   WHERE deleted = 0 AND dirty = 1 AND type IN ('rule','data_element','extension')
                   ORDER BY type, name`).all() as UnpublishedRow[];
}
